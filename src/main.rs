...
/// shaders and object
fn run(
    mut state: program_state,
    ref window: &glium::Display,
                    ...
                    glutin::WindowEvent::KeyboardInput { input, .. } => {
                        match input.virtual_keycode {
                            ...
                            Some(glutin::VirtualKeyCode::V) => if in_menu {
                                vertex_shader_src =
                                    shader_parser::read_file(&"vertex_shader.vert".to_string());
                                in_menu = false;
                            },
                            // while in_menu if f is hit reload fragment shader and exit menu
                            Some(glutin::VirtualKeyCode::F) => if in_menu {
                                fragment_shader_src =
                                    shader_parser::read_file(&"fragment_shader.frag".to_string());
                                in_menu = false;
                            },
...
fn draw(
    ...
    posTex: &Vec<object_parser::mat::PosTex>,
    normals: &Vec<object_parser::mat::Normal>,
    position: &[f32; 3],
    direction: &[f32; 3],
) -> () {
    //get the draw target
